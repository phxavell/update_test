...
def registrar_log(server_name, server_location, ping, download_speed, upload_speed):
    ...
    # Validação de aprovação/reprovação
    status = "APROVADO"
    if download_speed < 80:
        status = "REPROVADO: taxa de download de baixa qualidade"
    if upload_speed < 40:
        status = "REPROVADO: taxa de upload de baixa qualidade"
    ...
    with open(log_filename, 'a') as log_file:
        log_file.write(f"{timestamp} - Servidor: {server_name}, Localização: {server_location}, "
                       ...
                       f"Upload: {upload_speed:.2f} Mbps, Status: {status}\n")
